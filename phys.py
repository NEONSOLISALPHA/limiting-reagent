...
class Intro(Scene):
    def construct(self):
        ...
        Intro = TextMobject('\\emph{Formula for finding thickness of thread: }','\\emph{(Indirectly)}') #intro String initialisation
        ...
        coil = SVGMobject("/Volumes/Transcend/manim/myanims/media/designs/svg_images/Coil.svg") #Create Coil SVG Object
        coil.scale(1.4)
        
        coil_text = TextMobject('String Wrapped around a cylinder','i.e a coil').scale(0.75)
        ...
        self.play(MoveToTarget(coil),FadeOutAndShift(coil_text_vg,direction=DOWN))
        ...
        thickness = TextMobject('\\emph{thickness}','\\emph{of}','\\emph{string}')
        thickness.scale(0.8)
        ...
        self.play(ShowCreation(coil_bR),ShowCreation(coil_bL),ShowCreation(coil_bD))
        self.play(Write(no_of_turns_vg),Write(length_of_coil_vg),Write(thickness),run_time=1)
        self.wait()

        
        ### FORMULA START

        formula = TexMobject("\\emph{Thickness}",'\\emph{of}','\\emph{string}','=','{\\emph{length of coil}','\\over','\\emph{no. of turns}}')
        formula.scale(1.1)
        ...
        self.wait(2)
        self.play(*[Uncreate(i) for i in [no_of_turns_vg,thickness_vg,length_of_coil_vg,coil_bR,coil_bL,coil_bD,In_g,formula,coil]])
        self.wait(4)
