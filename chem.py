...
class Intro(Scene):
    def construct(self):
        ...
        q2_answer=TextMobject("The ","Excess reagent ","is the reactant in a chemical reaction which is present \n\tin a greater amount than necessary to react completely with the limiting reactant")
        ...
        self.wait(4)
        self.play(Uncreate(q),Uncreate(q_answer,run_time=1),Uncreate(q2_answer,run_time=1))
        self.wait(2)
...
class Bread(Scene):
    def construct(self):
        ...
        self.wait()
        equa=TextMobject("\\emph i\\emph f ","1 ","Cheese ","+ ","2 ","Bread ","\\ce{->} ","1 ","Cheese "," Sandwich")
        ...
        ce=TextMobject("Cheese ","+ ","6 ","Bread ","\\ce{->} ","3 ","Cheese "," Sandwiches")
        ced=TextMobject("Cheese ","+ ","6 ","Bread ","\\ce{->} ","more ","Cheese "," Sandwiches")
        ced.shift(RIGHT*0.7)
        ...
        self.wait()
        self.play(val.increment_value,900,runtime=2)#there is a bug where the increment is over or under executed so this value accounts for that
        self.wait()
        changes_1 =[
                   (0,1,2,3,4,5,6,7),
                   (0,1,2,3,4,5,6,7)
        ...
        self.wait()
        self.play(ReplacementTransform(br2,br1),ReplacementTransform(lim,exc))
        self.wait()
        self.play(*[Uncreate(i) for i in [eg,br1,br2,lim,exc,ced,dec,cross,equa]])
        self.wait()
...
class H2O(Scene):
    def construct(self):
        h2o=TexMobject("2","{H}_{2}","+","1","{O}_{2}","\\ce{->}","2","{H}_{2}{O}")
        h2ox= TexMobject("6","{H}_{2}","+","2","{O}_{2}","\\ce{->}","?")
        h2oy= TexMobject("6","{H}_{2}","+","2","{O}_{2}","\\ce{->}","4","{H}_{2}{O}")
        ...
        self.play(
                *[
                 ...
                 for i,j in zip(changes_[0],changes_[1]) #change zip(changes[0],changes[1]) to zip(uno,dos) if using multiple stages
                 ]
                 )
        self.wait()
        changes=[ 
                 (0,1,2,3,4,5,6,7),# this is to avoid rendering issueshile transforming
                 (0,1,2,3,4,5,6,7)
                ]
        self.wait(4)
        self.play(
